...
打开 **设置 → 模型**，点击某个提供方的 **编辑** 展开「模型目录」，每个模型条目下都有一个 **思考强度** 按钮：

1. 点击模型的 **思考强度**，在弹窗中勾选该模型支持的级别（不思考 / 极简 / 低 / 中 / 高 / 极高 / 最高），并为每个开启的级别填写发送给网关的线上值（默认与级别同名）；`不思考` 不需要线上值，表示“支持、但不发送任何参数”；
2. 点击 **保存**。修改会写入 `settings.yaml` 中 `llm-pi-ai.providers.<route>.models[].reasoningEfforts`，会话里的模型选择器随即按此提供级别。

也可以勾选 **该模型不支持思考**（写入 `reasoningEfforts: false`），或用 **清除配置** 移除该字段、回到内置目录的能力。只有 `off` 允许留空线上值，且必须至少声明一个非 `off` 的级别，与 pi-ai 适配器的解析规则一致；无效配置会在写入前被拒绝，不会破坏 `settings.yaml`。

**自动填充：** 通过 **获取可用模型** 添加的模型默认没有 `reasoningEfforts`，适配器会把它当作不思考。本补丁会在保存提供方后，为**没有任何 `reasoningEfforts` 的模型**自动填入默认思考强度 `{ low, medium, high }`，让新拉取的模型立刻可以思考；如需某个模型不思考，请把它设为 `reasoningEfforts: false`（显式 false 不会被自动填充）。默认级别可用每个模型条目下的思考强度按钮再微调。
